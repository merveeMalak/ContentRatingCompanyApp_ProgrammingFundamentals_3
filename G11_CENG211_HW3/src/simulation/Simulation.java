...
public class Simulation implements ISimulation {
    ...
    private void createCriticQueues() {
    ...
    private void organizeEvaluations(IGame gameToRate, IGameCritic gameCriticToRate) {
    ...
    //evaluates casual games, evaluationStatus==true means evaluation is done
    ...
    private void evaluateCasualGame(IGame gameToRate,IGameCritic gameCriticToRate) {
        ...
        for(int i=1; i<4;i++) {
            ...
            if(gameCriticToRate.getShift()>gameToRate.getDuration()) {
                sufficientCriticShift(gameCriticToRate, gameToRate, gameToRate.getDuration());
                if(i==3) {
                ...
                switch(i) {
                    ...
                    default:
                        insufficientCriticShift(gameCriticToRate, (gameToRate.getDuration()*(3-i)));
                        isShiftEnded = 2;
                        break;
                }
            }
            else {
                insufficientCriticShift(gameCriticToRate, (gameToRate.getDuration()*(4-i)));
                isShiftEnded = 2;
                break;
            }
        }
        ...
        if (evaluationStatus){
            organizeEvaluations(gameToRate,gameCriticToRate);
        }
    ...
    private void evaluateIndefiniteGame(IGame gameToRate,IGameCritic gameCriticToRate) {
        ...
        if (gameCriticToRate.getShift()>4) {
            sufficientCriticShift(gameCriticToRate, gameToRate, 4);
            evaluationStatus = true;
        }
            ...
            insufficientCriticShift(gameCriticToRate, 4);
            isShiftEnded = 2;
        }
    ...
    private void evaluateStoryGame(IGame gameToRate,IGameCritic gameCriticToRate) {
        ...
        if(gameCriticToRate.getShift()>gameToRate.getDuration()) {
            ...
            insufficientCriticShift(gameCriticToRate, gameToRate.getDuration());
            isShiftEnded = 2;
        }
    ...
    public void resetShifts() {
        ...
        for (IGameCritic criticsInProgress : gameCriticsInProgress) {
            criticsInProgress.setShift(criticsInProgress.getShift() + 8);
        }
        gameCriticQueue.addAll(gameCriticsEndedShifts);

        ArrayList<IGameCritic> tempGameCritic = new ArrayList<>();
        while (!(gameCriticQueue.isEmpty())) {
            tempGameCritic.add(gameCriticQueue.poll());
        }
        for (IGameCritic iGameCritic : tempGameCritic) {
            iGameCritic.setShift(8);
            gameCriticQueue.add(iGameCritic);
        }
    ...
    public void continueEvaluatingGames() {
        ...
        for (int i=0; i<gamesInProgress.size();i++) {
            ...
            if (gameCriticToRate.getShift()>0) {
                sufficientCriticShift(gameCriticToRate,gameToRate,0);
                evaluationStatus = true;
            }
                ...
                insufficientCriticShift(gameCriticToRate,0);
                isShiftEnded = 2;
            }
            redirectCritics(isShiftEnded,gameToRate,gameCriticToRate);
            if (evaluationStatus){
    ...
    private void sufficientCriticShift(IGameCritic gameCriticToRate, IGame gameToRate, int duration) {
    ...
    private void insufficientCriticShift(IGameCritic gameCriticToRate, int duration) {
        gameCriticToRate.setShift(gameCriticToRate.getShift()-duration);

    }

    //calls needed methods in the needed order to simulate 5 days of the company
    public void evaluateFiveDays() {
        for(int i=1; i<6; i++) {
    ...
    private void printGameRatings() {
        for (IGame evaluatedGame : evaluatedGames) {
            System.out.println(evaluatedGame.getName() + ", " + evaluatedGame.getEvaluateRate());
        }
    }
